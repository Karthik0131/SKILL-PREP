...
export async function registerStudent(studentData) {
  try {
    ...
    return await makeRequest('/api/students/register', {
      method: 'POST',
...
export async function getStudentProfile(rollNumber) {
  try {
    console.log(`Fetching profile for student with roll number: ${rollNumber}`);
    return await makeRequest(`/api/students/${rollNumber}`, {
      method: 'GET',
    });
  } catch (error) {
...
export async function getStudentQuizHistory(rollNumber) {
  try {
    ...
    const normalizedQuizHistory = quizHistory.map((quiz) => {
      if (quiz.category) {
...
export async function getStudentPerformance(rollNumber) {
  try {
    console.log(`Fetching performance for student with roll number: ${rollNumber}`);
    const data = await makeRequest(`/api/students/${rollNumber}/performance`, {
      method: 'GET',
    });
    ...
    const standardizeCategories = (categories) => {
      const result = {};
...
export async function getStudentRecommendations(rollNumber) {
  try {
    console.log(`Fetching recommendations for student with roll number: ${rollNumber}`);
    const data = await makeRequest(`/api/students/${rollNumber}/recommendations`, {
      method: 'GET',
    });
...
export async function getQuizzes() {
  return makeRequest('/api/quizzes');
}

export async function getQuizById(id) {
  return makeRequest(`/api/quizzes/${id}`);
}

export async function getQuestionById(questionId) {
  try {
    console.log(`Fetching question details for ID: ${questionId}`);
    return await makeRequest(`/api/questions/${questionId}`);
  } catch (error) {
...
export async function getQuizQuestions(quizId) {
  try {
    const response = await makeRequest(`/api/quizzes/${quizId}/questions`);
...
export async function submitStudentQuiz(rollNumber, quizData) {
  try {
    ...
    try {
      const result = await makeRequest(`/api/students/${rollNumber}/submit`, {
        method: 'POST',
...
export async function getQuizResults(quizId) {
  return makeRequest(`/api/results/${quizId}`);
}

export async function getStudentData(rollNumber) {
  try {
...
export async function updateResourceCompletion(rollNumber, resourceId, isCompleted) {
  try {
    if (!resourceId) {
    ...
    return makeRequest(`/api/students/${rollNumber}/resources/${resourceId}`, {
      method: 'PATCH',
